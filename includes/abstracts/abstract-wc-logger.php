 ...
 * @since      1.0.0
 *
 * @package    Dream_Encode_Test/includes/abstracts
 */
 ...
 *
 * @package  Dream_Encode\Test\Core\Abstracts\Dream_Encode_Test_Abstract_WC_Logger
 * @author   David Baumwald <david@dream-encode.com>
 */
class Dream_Encode_Test_Abstract_WC_Logger {
	...
	public static function log( $data, $level = 'debug' ) {
		...
		$log_level_setting = dream_encode_test_get_plugin_setting( 'log-level', 'off' );
		...
		if ( ! function_exists( 'wc_get_logger' ) ) {
			...
			error_log(
				sprintf(
					...
					__( '%1$s->%2$s depends on `wc_get_logger` which is bundled with the WooCommerce plugin.', 'dream-encode-test' ),
					__CLASS__,
					__METHOD__
				)
			);
