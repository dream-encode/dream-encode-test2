<?php
/**
 * Class Dream_Encode_Test_Abstract_Object_Data.
 *
 * @since 1.0.0
 */
...
if ( ! class_exists( 'Dream_Encode_Test_Abstract_Object_Data' ) ) {

	/**
	 * Class Dream_Encode_Test_Abstract_Object_Data
	 */
	abstract class Dream_Encode_Test_Abstract_Object_Data {
		/**
		 * ID.
		 *
		 * @var int
			...
			if ( $errors ) {
				...
				throw new Exception(
					sprintf(
						...
						__( 'The following functions %1$s do not exists in %2$s', 'dream-encode-test' ),
						implode( ',', $errors ),
						get_class( $this )
					)
				);
